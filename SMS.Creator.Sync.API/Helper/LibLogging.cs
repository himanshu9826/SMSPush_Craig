...
namespace SMS.Creator.Common.Helper
{
    public class LibLogging
    {
        ...
        /// <summary>
        /// Method to wrtie ex in log
        /// </summary>
        /// <param name="Source"></param>
        /// <param name="Code"></param>
        /// <param name="ex"></param>

        public static void WriteSMSToLog(string Source, string Code, string ex = null)
        {
            string ErrorFolder = ConfigurationManager.AppSettings["LogPath"].ToString();
            if (!Directory.Exists(ErrorFolder))
            {
            ...
            try
            {
                ...
                if (addLog == 1)
                {
                    using (StreamWriter writer = new StreamWriter(filePath, true))
                    {
                        string message = ex != null ? ex : string.Empty;
                        string stackTrace = ex != null ? ex : string.Empty;
                        writer.WriteLine("Methodl: " + Code + " | Source: " + Source + " | Message :" + message + "<br/>" + Environment.NewLine + "StackTrace :" + stackTrace +
                           "" + Environment.NewLine + "Date :" + DateTime.Now.ToString());
                        writer.WriteLine(Environment.NewLine + "-----------------------------------------------------------------------------" + Environment.NewLine);
                    }
